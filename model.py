...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # output projection
        self.c_proj = nn.Linear(config.d_resid, config.d_model, bias=config.bias)
        # regularization
...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.c_fc    = nn.Linear(config.d_resid, 4 * config.d_resid, bias=config.bias)
        self.gelu    = nn.GELU()
        self.c_proj  = nn.Linear(4 * config.d_resid, config.d_model, bias=config.bias)
        self.dropout = nn.Dropout(config.dropout)
        self.config = config
        self.ln_2 = LayerNorm(config.d_resid, bias=config.bias)
...
class Unembed(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.ln = LayerNorm(config.d_output, bias=config.bias)
        self.W_U = nn.Linear(config.d_output, config.vocab_size, bias=config.bias)
        torch.nn.init.normal_(self.W_U.weight, mean=0.0, std=0.02)
        
        
    def forward(self, x):
        x = x[:, :, -self.config.d_output:]
        x = self.ln(x)
        x = self.W_U(x)
        return x
...
class GPT(nn.Module):
    ...
    def get_num_params(self, non_embedding=False):
        ...
        """
        if non_embedding:
            raise NotImplementedError("non_embedding=True not supported")
